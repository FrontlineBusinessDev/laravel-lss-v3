...
class TraineesController extends BaseController
{
    ...
    protected array $filterable = ['status', 'batch_id', 'school_id', 'academic_level_id'];

    protected array $exactFilters = ['status', 'batch_id', 'school_id', 'academic_level_id'];
    ...
    /**
     * academic_industry_id/academic_program_id live on the batch, not
     * app_trainees, so they can't ride BaseController's generic flat-column
     * filter loop — applied here via whereHas('batch', ...) instead.
     * academic_level_id is trainee-owned, so it rides the generic filter loop.
     */
    protected function newQuery(): Builder
    {
        ...
        foreach (['academic_industry_id', 'academic_program_id'] as $batchFilter) {
