...
export function BatchesProvider({ children }: { children: ReactNode }) {
  const [batches, setBatches] = useState<Batch[]>(() =>
    initialBatches.map((b) => ({ ...b, trainees: initialTrainees.filter((t) => t.batchNo === b.batchNo && !t.archived).length })),
  )
  ...
  const updateBatch = (id: string, input: CreateBatchInput) => {
    setBatches((prev) => prev.map((b) => (b.id === id ? { ...b, ...input } : b)))
  }

  const completeBatch = (id: string) => {
    setBatches((prev) => prev.map((b) => (b.id === id ? { ...b, status: 'completed' as StatusKind } : b)))
  }

  const dissolveBatch = (id: string, remarks: string) => {
    setBatches((prev) =>
      prev.map((b) => (b.id === id ? { ...b, status: 'dissolved' as StatusKind, dissolvedRemarks: remarks } : b)),
    )
  }
  ...
  const recomputeCounts = (list: Trainee[]) => {
    setBatches((prev) =>
      prev.map((b) => ({ ...b, trainees: list.filter((t) => t.batchNo === b.batchNo && !t.archived).length })),
    )
  }

  const transferTrainee = (traineeId: string, toBatchNo: string) => {
    setTrainees((prev) => {
      const next = prev.map((t) => (t.id === traineeId ? { ...t, batchNo: toBatchNo } : t))
  ...
  const terminateTrainee = (traineeId: string, remarks: string) => {
    setTrainees((prev) => {
      const next = prev.map((t) =>
        t.id === traineeId ? { ...t, status: 'terminated' as StatusKind, terminationRemarks: remarks } : t,
      )
      recomputeCounts(next)
      return next
    })
  ...
  const archiveTrainee = (traineeId: string) => {
    setTrainees((prev) => {
      const next = prev.map((t) =>
        t.id === traineeId ? { ...t, archived: true, statusBeforeArchive: t.status } : t,
      )
      recomputeCounts(next)
      return next
    })
  ...
  const restoreTrainee = (traineeId: string) => {
    setTrainees((prev) => {
      const next = prev.map((t) =>
        t.id === traineeId ? { ...t, archived: false, status: t.statusBeforeArchive ?? t.status } : t,
      )
      recomputeCounts(next)
      return next
    })
  ...
  const setEvaluationAccessOverride = (traineeId: string, allowed: boolean) => {
    setTrainees((prev) => prev.map((t) => (t.id === traineeId ? { ...t, evaluationAccessOverride: allowed } : t)))
  }

  const updateTrainee = (traineeId: string, patch: Partial<Trainee>) => {
    setTrainees((prev) => prev.map((t) => (t.id === traineeId ? { ...t, ...patch } : t)))
  }
