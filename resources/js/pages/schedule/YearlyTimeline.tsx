...
export function YearlyTimeline({
  ...
  return <div className="rounded-lg border border-neutral-200 bg-white" data-cy="yearly-timeline-div-1">
      <div className="flex items-center justify-between border-b border-neutral-200 p-3.5" data-cy="yearly-timeline-div-2">
        <h2 className="text-sm font-semibold text-ink" data-cy="yearly-timeline-h2-yearly-batch-timeline">Yearly batch timeline</h2>
        <div className="flex items-center gap-1" data-cy="yearly-timeline-div-4">
          <button onClick={() => onYearChange(year - 1)} aria-label="Previous year" className="flex h-7 w-7 items-center justify-center rounded-md text-neutral-400 transition-colors hover:bg-neutral-100 hover:text-neutral-600" data-cy="yearly-timeline-button-previous-year">
            <ChevronLeft size={14} data-cy="yearly-timeline-chevron-left-6" />
          </button>
          <span className="w-12 text-center text-sm font-semibold text-ink" data-cy="yearly-timeline-span-7">{year}</span>
          <button onClick={() => onYearChange(year + 1)} aria-label="Next year" className="flex h-7 w-7 items-center justify-center rounded-md text-neutral-400 transition-colors hover:bg-neutral-100 hover:text-neutral-600" data-cy="yearly-timeline-button-next-year">
            <ChevronRight size={14} data-cy="yearly-timeline-chevron-right-9" />
          </button>
        </div>
      </div>

      {/* Desktop / tablet Gantt */}
      <div className="hidden overflow-x-auto sm:block lss-scrollbar" data-cy="yearly-timeline-div-10">
        <div className="min-w-[860px] p-3.5" data-cy="yearly-timeline-div-11">
          {/* Month header */}
          <div className="mb-2 grid grid-cols-12 border-b border-neutral-100 pb-2 pl-44" data-cy="yearly-timeline-div-12">
            {MONTH_LABELS.map(m => <div key={m} className="text-center text-[11px] font-medium text-neutral-400" data-cy="yearly-timeline-div-13">
                {m}
              </div>)}
          </div>

          <div className="flex flex-col gap-1.5" data-cy="yearly-timeline-div-14">
            {visible.map(entry => {
            ...
            {visible.length === 0 && <div className="py-10 text-center text-sm text-neutral-400" data-cy="yearly-timeline-div-no-schedules-match-your-filters-for">No schedules match your filters for {year}.</div>}
          </div>
        </div>
      </div>

      {/* Mobile card list */}
      <div className="flex flex-col gap-2 p-3 sm:hidden" data-cy="yearly-timeline-div-24">
        {visible.map(entry => {
        const color = getSchoolColor(entry.primarySchool);
        ...
        {visible.length === 0 && <div className="rounded-lg border border-dashed border-neutral-200 py-8 text-center text-sm text-neutral-400" data-cy="yearly-timeline-div-no-schedules-match-your-filters-for-2">
