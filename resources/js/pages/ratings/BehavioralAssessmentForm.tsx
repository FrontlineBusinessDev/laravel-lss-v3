...
export function BehavioralAssessmentForm() {
  ...
  const activeQuestions = useMemo(
    () => behavioralQuestions.filter((q) => q.status === 'active').sort((a, b) => a.order - b.order),
    [],
  )
  ...
  const [formFor, setFormFor] = useState<{ traineeId: string; traineeName: string } | null>(null)
  ...
  function submitForm() {
    ...
    const historyEntry = { answers, overallComments, recommendation, evaluator: currentUser.name, ratedAt }
    ...
    setRatings((prev) => {
      if (existing) {
        return prev.map((r) =>
          r.id === existing.id
            ? { ...r, answers, overallComments, recommendation, evaluator: currentUser.name, ratedAt, history: [...r.history, historyEntry] }
            : r,
        )
      }
  ...
  return (
    <div>
      <div className="mb-4 no-print">
        <p className="text-sm text-neutral-500">Evaluate trainees using the Trainer Evaluation for Trainees questionnaire.</p>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-end gap-3 rounded-lg border border-neutral-200 bg-white p-3.5 no-print">
        <div className="w-56">
          ...
          <Dropdown
            ...
            onChange={(v) => setBatchNo(v === 'Select batch' ? '' : v)}
          />
        </div>
        {batchNo && (
          <StatCard
            ...
            <table className="w-full min-w-[640px] border-collapse text-sm">
              <thead>
                <tr className="bg-neutral-50 text-left text-xs font-medium text-neutral-500">
                  ...
                  <th className="px-4 py-2.5 font-medium">Overall score</th>
                ...
                {batchTrainees.map((tr) => {
                  ...
                  return (
                    <tr key={tr.id} className="border-t border-neutral-100 align-top">
                      ...
                      <td className="px-4 py-3">{existing ? <RatingStars value={avg} /> : <span className="text-xs text-neutral-400">—</span>}</td>
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap items-center justify-end gap-1.5">
                          <Button size="sm" variant="primary" icon={ClipboardCheck} onClick={() => openForm(tr.id, tr.name)}>
                          ...
                          {existing && (
                            <>
                              ...
                              <Button size="sm" variant="secondary" icon={Printer} onClick={() => printRating(existing)}>
                                Print
                              </Button>
                            </>
                          )}
            ...
            return (
              <div key={tr.id} className="rounded-lg border border-neutral-200 bg-white p-3.5">
                <div className="mb-2 flex items-start justify-between gap-2">
                  ...
                  {existing ? <StatusBadge status="completed" /> : (
                    <span className="inline-flex items-center rounded-pill bg-neutral-100 px-2.5 py-0.5 text-xs font-medium text-neutral-500">
                      Not evaluated
                    </span>
                  )}
          ...
          <>
            <div className="mb-5 flex flex-col gap-5">
              {sections.map(([section, qs]) => (
                <div key={section}>
                  <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-brand-600">{section}</div>
                        ...
                        {q.type === 'rating' ? (
                          <div className="flex items-center justify-between gap-3 rounded-md border border-neutral-200 p-3">
                            ...
                            <RatingStars
                              ...
                              onChange={(v) => setDraftScores((prev) => ({ ...prev, [q.id]: v }))}
                              size={18}
                            />
                          </div>
                        ) : (
                          <div className="rounded-md border border-neutral-200 p-3">
                            ...
                            <textarea
                              ...
                              onChange={(e) => setDraftTexts((prev) => ({ ...prev, [q.id]: e.target.value }))}
            ...
            <div className="flex gap-2">
              <Button variant="secondary" className="flex-1" onClick={() => setFormFor(null)}>
                Cancel
              </Button>
              <Button variant="primary" className="flex-1" onClick={submitForm} disabled={activeQuestions.length === 0}>
      ...
      {/* History modal */}
      <Modal open={!!historyFor} onClose={() => setHistoryFor(null)} title={`Evaluation history \u2013 ${historyFor?.traineeName ?? ''}`} maxWidth={520}>
        {historyFor && (
          <div className="flex flex-col gap-2.5">
            <p className="mb-1 text-xs text-neutral-500">{historyFor.batchNo}</p>
            {[...historyFor.history].reverse().map((h, i) => (
              <div key={i} className="rounded-md border border-neutral-200 p-3">
                ...
                {h.overallComments && <p className="text-xs text-neutral-600">{h.overallComments}</p>}
                {h.recommendation && <p className="mt-1 text-xs italic text-neutral-500">Recommendation: {h.recommendation}</p>}
      ...
      {printFor && (
        <BehavioralSheetPrint
          ...
          rating={printFor}
          questions={behavioralQuestions.filter((q) => printFor.answers.some((a) => a.questionId === q.id))}
