...
export function BehavioralRatingPage() {
  ...
  return (
    <div>
      <div className="mb-4 flex gap-1 rounded-md border border-neutral-200 bg-neutral-50 p-1 no-print w-fit">
        {SUB_TABS.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={cn(
              ...
              tab === t ? 'bg-white text-ink shadow-card' : 'text-neutral-500 hover:text-neutral-700',
            )}
          >
            {t}
          </button>
      ...
      {tab === 'Assessment Form' ? <BehavioralAssessmentForm /> : <BehavioralAssessmentSetup />}
    </div>
  )
}
