...
type PendingAction = {
    type: 'suspend' | 'reactivate' | 'archive' | 'restore' | 'delete' | 'reset';
    user: AppUser;
} | null;
...
export function UsersTab() {
    ...
    function runConfirmed() {
        ...
        switch (type) {
            ...
            case 'archive':
                setUsers((prev) =>
                    prev.map((u) =>
                        u.id === user.id ? { ...u, status: 'archived' } : u,
                    ),
                );
    ...
    > = {
        reset: {
            ...
            description: (u) =>
                `Send a password reset link to ${u.email}? They\u2019ll be able to set a new password from that link.`,
        },
        suspend: {
            title: 'Suspend user',
            tone: 'danger',
            ...
            description: (u) =>
                `${u.name} will immediately lose access to the system. You can reactivate this account anytime.`,
        },
        reactivate: {
            ...
            description: (u) =>
                `${u.name} will regain access to sign in to the system.`,
        },
        archive: {
            title: 'Archive user',
            tone: 'default',
            ...
            description: (u) =>
                `${u.name} will be moved to archived records. You can restore this account later.`,
        },
        restore: {
            title: 'Restore user',
            tone: 'default',
        ...
        <div>
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
                    <div className="relative w-full sm:max-w-[240px]">
                        ...
                        <input
                            ...
                            className="h-9 w-full rounded-md border border-neutral-200 pr-2.5 pl-8 text-sm shadow-card transition-colors hover:border-neutral-300 hover:shadow-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 focus:outline-none"
                        />
                    </div>
                            ...
                            <div className="flex items-start justify-between gap-2">
                                <div className="min-w-0">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        {isSelf && (
                                            <span className="shrink-0 rounded-pill bg-brand-50 px-1.5 py-0.5 text-[10px] font-medium text-brand-700">
                                                You
                                            </span>
                                        )}
                                    </div>
            ...
            {pending && (
                <ConfirmDialog
                    ...
                    description={dialogCopy[pending.type].description(
                        pending.user,
                    )}
                    ...
                    confirmDisabled={
                        pending.type === 'delete' &&
                        isLastActiveAdmin(pending.user)
                    }
                />
            )}
        </div>
