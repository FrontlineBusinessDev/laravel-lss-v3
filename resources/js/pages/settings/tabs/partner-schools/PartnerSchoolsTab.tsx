...
import { Plus, Search, Pencil, Archive, ArchiveRestore, Trash2, Building2 } from 'lucide-react'
...
const SWATCHES = ['bg-brand-50 text-brand-700', 'bg-success-50 text-success-800', 'bg-warning-50 text-warning-800', 'bg-neutral-100 text-neutral-500']

type PendingAction = { type: 'archive' | 'restore' | 'delete'; school: PartnerSchool } | null
...
export function PartnerSchoolsTab() {
  ...
  const filtered = useMemo(() => {
    ...
    return schools.filter((s) => {
      const matchesQuery = !q || s.name.toLowerCase().includes(q) || s.abbr.toLowerCase().includes(q) || s.contactPerson.toLowerCase().includes(q)
  ...
  function handleSave(values: SchoolFormValues) {
    if (editing) {
      setSchools((prev) => prev.map((s) => (s.id === editing.id ? { ...s, ...values } : s)))
      showToast(`${values.name} was updated.`, 'success')
    } else {
      const newSchool: PartnerSchool = { id: `s${Date.now()}`, trainees: 0, status: 'active', ...values }
  ...
  function runConfirmed() {
    ...
    if (type === 'archive') {
      setSchools((prev) => prev.map((s) => (s.id === school.id ? { ...s, status: 'archived' } : s)))
      showToast(`${school.name} was archived.`, 'success')
    } else if (type === 'restore') {
      setSchools((prev) => prev.map((s) => (s.id === school.id ? { ...s, status: 'active' } : s)))
  ...
  return (
    <div>
      <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <span className="text-xs text-neutral-500">{filtered.length} of {schools.length} partner schools</span>
              ...
              {filtered.map((s, i) => (
                <tr
                  ...
                  className={cn(
                    ...
                    s.status === 'archived' && 'text-neutral-400',
                  )}
                >
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      <div className={cn('flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded-md text-[10px] font-semibold', SWATCHES[i % SWATCHES.length])}>
                        {s.logoUrl ? <img src={s.logoUrl} alt="" className="h-full w-full object-cover" /> : s.abbr}
                      </div>
                      <span className={cn('font-medium', s.status === 'archived' ? 'text-neutral-400' : 'text-ink')}>{s.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-2.5 text-neutral-600">{s.contactPerson}</td>
                  <td className="px-4 py-2.5 text-xs text-neutral-500">{s.email}</td>
                  <td className="px-4 py-2.5 font-medium text-ink">{traineeCountBySchool.get(s.name) ?? 0}</td>
                  <td className="px-4 py-2.5 text-right">
                    <RowMenu
                      actions={[
                        { label: 'Edit school', icon: Pencil, onClick: () => openEdit(s) },
                        s.status === 'archived'
                          ? { label: 'Restore', icon: ArchiveRestore, onClick: () => setPending({ type: 'restore', school: s }) }
                          : { label: 'Archive', icon: Archive, onClick: () => setPending({ type: 'archive', school: s }) },
                        { label: 'Delete', icon: Trash2, danger: true, onClick: () => setPending({ type: 'delete', school: s }) },
                      ]}
                    />
                  </td>
                </tr>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-10 text-center text-sm text-neutral-400">
                    <Building2 size={20} className="mx-auto mb-2 text-neutral-300" />
      ...
      <div className="flex flex-col gap-2 sm:hidden">
        {filtered.map((s, i) => (
          <div key={s.id} className={cn('rounded-lg border border-neutral-200 bg-white p-3.5', s.status === 'archived' && 'opacity-60')}>
            <div className="flex items-start justify-between gap-2">
              <button onClick={() => openEdit(s)} className="flex min-w-0 flex-1 items-center gap-2 text-left">
                <div className={cn('flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-md text-[10px] font-semibold', SWATCHES[i % SWATCHES.length])}>
                  {s.logoUrl ? <img src={s.logoUrl} alt="" className="h-full w-full object-cover" /> : s.abbr}
                </div>
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-ink">{s.name}</p>
                  <p className="truncate text-xs text-neutral-500">{s.contactPerson}</p>
                </div>
              </button>
              <RowMenu
                actions={[
            ...
            <div className="mt-2 flex items-center justify-between text-xs text-neutral-500">
              ...
              <span className="shrink-0 font-medium text-ink">{traineeCountBySchool.get(s.name) ?? 0} trainees</span>
            </div>
          </div>
        ))}
        {filtered.length === 0 && (
      ...
      <AddSchoolModal open={modalOpen} onClose={() => setModalOpen(false)} onSave={handleSave} initial={editing} />
      ...
      {pending && (
        <ConfirmDialog
          ...
          title={pending.type === 'archive' ? 'Archive school' : pending.type === 'restore' ? 'Restore school' : 'Delete school'}
          ...
          confirmDisabled={pending.type === 'delete' && (traineeCountBySchool.get(pending.school.name) ?? 0) > 0}
          description={
