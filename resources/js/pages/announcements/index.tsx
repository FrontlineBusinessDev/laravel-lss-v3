...
import { AddAnnouncementModal, type AnnouncementFormValues } from '@/pages/announcements/AddAnnouncementModal'

type PendingAction = { type: 'archive' | 'restore' | 'delete'; announcement: Announcement } | null

function resolveRecipientCount(
  values: Pick<AnnouncementFormValues, 'audience' | 'batchNo' | 'groupTraineeNames'>,
  trainees: Trainee[],
): number {
...
export default function AnnouncementsPage() {
  ...
  const filtered = useMemo(() => {
    return items
      .filter((a) => statusFilter === 'All statuses' || a.status === statusFilter.toLowerCase())
  ...
  function runConfirmed() {
    ...
    if (type === 'archive') {
      setItems((prev) => prev.map((a) => (a.id === announcement.id ? { ...a, status: 'archived' } : a)))
      showToast(`"${announcement.title}" was archived.`, 'success')
    } else if (type === 'restore') {
      setItems((prev) => prev.map((a) => (a.id === announcement.id ? { ...a, status: 'active' } : a)))
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-500">{activeCount} active of {items.length} total announcements</p>
        </div>
      ...
      <div className="mb-4 flex items-start gap-2 rounded-lg border border-brand-100 bg-brand-50 px-3.5 py-2.5 text-xs text-brand-700">
        ...
        <span>
          Signed in as <strong>{currentUser.name}</strong> ({currentUser.role}). Admins can post to any audience and archive, restore, or permanently delete any
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="text-xs text-neutral-400">{filtered.length} of {items.length}</span>
      </div>
      ...
      <div className="divide-y divide-neutral-100 overflow-hidden rounded-lg border border-neutral-200 bg-white">
        {filtered.map((a) => (
          <div key={a.id} className="flex items-start justify-between gap-3 px-4 py-3.5">
            <div className="flex items-start gap-3">
              <span
                className={cn(
                  ...
                  a.status === 'archived' ? 'bg-neutral-100 text-neutral-400' : 'bg-brand-50 text-brand-600',
                )}
              >
                <Megaphone size={14} />
              </span>
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className={cn('text-sm font-medium', a.status === 'archived' ? 'text-neutral-400' : 'text-ink')}>{a.title}</span>
                  <StatusBadge status={a.status} />
                </div>
                <p className={cn('mt-0.5 max-w-xl text-xs', a.status === 'archived' ? 'text-neutral-400' : 'text-neutral-500')}>{a.body}</p>
                <p className="mt-1 text-xs text-neutral-400">
                  Posted by {a.postedBy} · {new Date(a.postedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            ...
            <RowMenu
              actions={[
                a.status === 'archived'
                  ? { label: 'Restore', icon: ArchiveRestore, onClick: () => setPending({ type: 'restore', announcement: a }) }
                  : { label: 'Archive', icon: Archive, onClick: () => setPending({ type: 'archive', announcement: a }) },
                { label: 'Delete', icon: Trash2, danger: true, onClick: () => setPending({ type: 'delete', announcement: a }) },
              ]}
            />
          </div>
        ))}
        {filtered.length === 0 && (
          <div className="px-4 py-10 text-center text-sm text-neutral-400">
            <Megaphone size={20} className="mx-auto mb-2 text-neutral-300" />
      ...
      <AddAnnouncementModal
        ...
        trainees={trainees}
        resolveRecipientCount={resolveRecipientCount}
      />
      ...
      {pending && (
        <ConfirmDialog
          ...
          title={pending.type === 'archive' ? 'Archive announcement' : pending.type === 'restore' ? 'Restore announcement' : 'Delete announcement'}
