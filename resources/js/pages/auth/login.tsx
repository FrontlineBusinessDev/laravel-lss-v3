...
export default function LoginPage({ status }: { status?: string }) {
    ...
    return (
        <AuthLayout data-cy="login-auth-layout-1">
            <h1
                className="mb-1 text-xl font-semibold text-ink"
                data-cy="login-h1-log-in-to-your-account"
            >
                Log in to your account
            </h1>
            <p
                className="mb-6 text-sm text-neutral-500"
                data-cy="login-p-enter-your-credentials-to-continue"
            >
            ...
            {status && (
                <div
                    className="mb-4 rounded-md bg-success-50 px-3 py-2 text-center text-xs font-medium text-success-600"
                    data-cy="login-div-4"
                >
                    {status}
                </div>
            ...
            <form onSubmit={handleSubmit} data-cy="login-form-submit">
                ...
                <div className="mb-2" data-cy="login-div-10">
                    ...
                    <div className="relative" data-cy="login-div-12">
                        <input
                            ...
                            className="h-10 w-full rounded-md border border-neutral-200 bg-white px-3 pr-9 text-sm text-ink transition-colors placeholder:text-neutral-400 hover:border-neutral-300 focus:border-brand-500 focus:ring-2 focus:ring-brand-100 focus:outline-none"
                            data-cy="login-input-enter-your-password"
                        />
                        <button
                            type="button"
                            ...
                            data-cy="login-button-button"
                        >
                            {showPassword ? (
                                <EyeOff size={16} data-cy="login-eye-off-15" />
                            ) : (
                                <Eye size={16} data-cy="login-eye-16" />
                            )}
                        </button>
                    </div>
                    {errors.password && (
                ...
                <div className="mb-5 text-right" data-cy="login-div-18">
                    <Link
                        ...
                        data-cy="login-link-forgot-password"
                    >
                        Forgot password?
                    </Link>
                </div>
