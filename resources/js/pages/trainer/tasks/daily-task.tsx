...
async function loadTraineeOptions(q: string): Promise<FieldOption[]> {
    const res = await apiFetchJson<PersonOption[]>(
        `/trainer/trainees/lookup?status=active&per_page=50&q=${encodeURIComponent(q)}`,
    );
...
/**
 * Same report as the admin Daily Task Sheet — the shared /tasks/daily-task/*
 * API is already batch-scoped server-side (DailyTaskController::filteredQuery()).
 * Only the batch/trainee filter picker sources are swapped for trainer-scoped
 * ones so the dropdowns never list something outside their assigned batches.
 */
        ...
        <div>
            ...
            <div className="no-print">
                <DataTableCardField<ApiDailyTaskRow>
                    ...
                    apiQueryKey="trainer-daily-task-sheet"
                    columns={columns}
                    defaultSortBy="date"
