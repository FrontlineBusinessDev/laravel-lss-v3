...
// Batch column dropped — every row visible here already belongs to one of
// this trainer's assigned batches (LeaveRequestController::newQuery()).
const columns = baseColumns.filter((col) => col.key !== 'batch_id');

/**
 * Approve/decline/view leave requests from this trainer's assigned batches
 * only — the shared /leave JSON API scopes rows (pending-first + recency)
 * and re-checks batch assignment on every approve/decline
    ...
    return (
        <TrainerLayout title="Leave">
            <p className="mb-4 text-sm text-neutral-500">
                Review, approve, or decline leave requests from your assigned
                batches.
            </p>

            <DataTableCardField<LeaveRequests>
                apiUrl="/leave"
                apiQueryKey="trainer-leave-requests"
                columns={columns}
