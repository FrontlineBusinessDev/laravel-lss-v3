...
const EMPTY: CitationFormValues = { title: '', appliesTo: 'Trainee', bodyText: '', critical: false }
...
export function AddEditCitationModal({ open, onClose, onSave, initial }: AddEditCitationModalProps) {
  const isEdit = !!initial
  const [values, setValues] = useState<CitationFormValues>(EMPTY)
  const [errors, setErrors] = useState<{ title?: string; bodyText?: string }>({})

  useEffect(() => {
    if (open) {
      setValues(
        initial
          ? { title: initial.title, appliesTo: initial.appliesTo, bodyText: initial.bodyText, critical: !!initial.critical }
          : EMPTY,
      )
      setErrors({})
    }
  ...
  function insertToken(token: string) {
    setValues((v) => ({ ...v, bodyText: v.bodyText ? `${v.bodyText} ${token}` : token }))
  }
  ...
  function handleSubmit() {
    ...
    onSave({ title: values.title.trim(), appliesTo: values.appliesTo, bodyText: values.bodyText.trim(), critical: values.critical })
  }
  ...
  return (
    ...
    >
        ...
        <div className="flex overflow-hidden rounded-md border border-neutral-200">
          {(['Trainee', 'Seminar', 'Both'] as const).map((opt) => (
            <button
              key={opt}
              type="button"
              onClick={() => setValues((v) => ({ ...v, appliesTo: opt }))}
              className={cn(
                'flex-1 px-2.5 py-1.5 text-xs font-medium transition-colors',
                values.appliesTo === opt ? 'bg-brand-500 text-white' : 'bg-white text-neutral-600 hover:bg-neutral-50',
              )}
            >
              {opt}
            </button>
      ...
      <TextAreaField
        ...
        onChange={(e) => {
          setValues((v) => ({ ...v, bodyText: e.target.value }))
          setErrors((er) => ({ ...er, bodyText: undefined }))
        }}
      />
      {errors.bodyText && <p className="-mt-2.5 mb-2 text-xs font-medium text-danger-600">{errors.bodyText}</p>}
      ...
      <label className="mb-5 -mt-2 flex cursor-pointer items-start gap-2 rounded-md bg-neutral-50 px-3 py-2.5 text-xs text-neutral-600">
        ...
        />
        <span>
          <strong className="text-neutral-700">Mark as critical.</strong> Critical citations are protected from permanent
