...
export function QuestionsPanel({
    ...
    const modal = useGlobalModal<EvaluationQuestion | null>(
        'questionnaire',
        null,
    );
    ...
    const effectiveActiveSet = sets.includes(activeSet)
        ? activeSet
        : (sets[0] ?? '');
    const scoped = useMemo(
        () =>
            questions.filter(
                (q) =>
                    q.category === category &&
                    q.questionSet === effectiveActiveSet,
            ),
    ...
    return (
        <div
            ...
            data-cy="questions-panel-div-1"
        >
            <div
                className="flex flex-col gap-3 border-b border-neutral-200 p-4 sm:flex-row sm:items-center sm:justify-between"
                data-cy="questions-panel-div-2"
            >
                <div data-cy="questions-panel-div-3">
                    <h2
                        className="text-sm font-semibold text-ink"
                        data-cy="questions-panel-h2-evaluation-questions"
                    >
                        {category === 'Trainer' ? 'Trainer' : 'Seminar'}{' '}
                        evaluation questions
                    </h2>
                    <p
                        className="text-xs text-neutral-500"
                        data-cy="questions-panel-p-active-of"
                    >
                        {activeCount} active of {scoped.length} total in &ldquo;
                        {effectiveActiveSet}&rdquo; &middot; used by{' '}
                        {category === 'Trainer'
                ...
                <Button
                    ...
                    disabled={!effectiveActiveSet}
                    data-cy="questions-panel-button-set-editing"
                >
                    Add question
                </Button>
            </div>
            ...
            <div
                className="flex flex-wrap items-center gap-1.5 border-b border-neutral-100 p-3"
                data-cy="questions-panel-div-7"
            >
                {sets.map((s) => (
                    <button
                        key={s}
                        ...
                        className={cn(
                            ...
                            effectiveActiveSet === s
                                ? 'bg-brand-500 text-white'
                                : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200',
                        )}
                        data-cy="questions-panel-button-set-active-set"
                    >
                        {s}
                    </button>
                ))}
                ...
                {addingSet ? (
                    ...
                    >
                        <input
                            ...
                            className="h-7 w-44 rounded-pill border border-neutral-200 px-3 text-xs focus:border-brand-500 focus:ring-2 focus:ring-brand-100 focus:outline-none"
                            data-cy="questions-panel-input-text"
                        />
                        <button
                            onClick={submitNewSet}
                            className="text-success-700 rounded-full p-1 hover:bg-success-50"
                            aria-label="Confirm new set"
                            data-cy="questions-panel-button-confirm-new-set"
                        >
                            <Check
                                size={14}
                                data-cy="questions-panel-check-12"
                            />
                        </button>
                        <button
                            onClick={() => {
                            ...
                            aria-label="Cancel"
                            data-cy="questions-panel-button-cancel"
                        >
                    ...
                    <button
                        ...
                        data-cy="questions-panel-button-set-adding-set"
                    >
                        <FolderPlus
                            size={12}
                            data-cy="questions-panel-folder-plus-16"
                        />{' '}
                        Add set
                    </button>
                )}
            ...
            <div
                className="flex flex-col gap-2 border-b border-neutral-100 p-3 sm:flex-row sm:items-center"
                data-cy="questions-panel-div-17"
            >
                <div
                    className="relative w-full flex-1 sm:max-w-xs"
                    data-cy="questions-panel-div-18"
                >
                    <Search
                        size={14}
                        className="pointer-events-none absolute top-1/2 left-2.5 -translate-y-1/2 text-neutral-400"
                        data-cy="questions-panel-search-19"
                    />
                    <input
                        type="text"
                        value={query}
                        ...
                        data-cy="questions-panel-input-search-questions-or-author"
                    />
                </div>
                <div
                    className="w-full sm:w-40"
                    data-cy="questions-panel-div-21"
                >
                    <Dropdown
                        options={['All statuses', 'Active', 'Archived']}
                        value={statusFilter}
                        onChange={setStatusFilter}
                        data-cy="questions-panel-dropdown-set-status-filter"
                    />
                </div>
                {(query || statusFilter !== 'All statuses') && (
                    <button
                        ...
                        data-cy="questions-panel-button-set-query"
                    >
                        <X size={13} data-cy="questions-panel-x-24" /> Clear
                    </button>
                )}
                <span
                    className="text-xs text-neutral-400 sm:ml-auto"
                    data-cy="questions-panel-span-of"
                >
            ...
            <div
                ...
                data-cy="questions-panel-div-26"
            >
                {grouped.map(({ section, items }) => (
                    <div key={section} data-cy="questions-panel-div-27">
                        <div
                            className="bg-neutral-50 px-4 py-1.5 text-[11px] font-semibold tracking-wide text-neutral-500 uppercase"
                            data-cy="questions-panel-div-28"
                        >
                            {section}
                        </div>
                        {items.map((q) => (
                            <div
                                ...
                                data-cy="questions-panel-div-29"
                            >
                                <div
                                    className="min-w-0"
                                    data-cy="questions-panel-div-30"
                                >
                                    <div
                                        className="flex flex-wrap items-center gap-1.5"
                                        data-cy="questions-panel-div-31"
                                    >
                                        <span
                                            className={cn(
                                                'text-sm',
                                            ...
                                            data-cy="questions-panel-span-32"
                                        >
                                            {q.question}
                                        </span>
                                        <span
                                            className="inline-flex items-center rounded-pill bg-neutral-100 px-1.5 py-0.5 text-[10px] font-medium text-neutral-500"
                                            data-cy="questions-panel-span-33"
                                        >
                                            {q.type === 'text'
                                                ? 'Written'
                                                : 'Rating'}
                                        ...
                                        {q.critical && (
                                            <span
                                                className="inline-flex items-center gap-1 rounded-pill bg-warning-50 px-1.5 py-0.5 text-[10px] font-medium text-warning-800"
                                                data-cy="questions-panel-span-critical"
                                            >
                                                <Lock
                                                    size={9}
                                                    data-cy="questions-panel-lock-35"
                                                />{' '}
                                                Critical
                                            </span>
                                        )}
                                        {q.status === 'archived' && (
                                            <span
                                                className="inline-flex items-center rounded-pill bg-neutral-100 px-1.5 py-0.5 text-[10px] font-medium text-neutral-500"
                                                data-cy="questions-panel-span-archived"
                                            >
                                                Archived
                                            </span>
                                        )}
                                    ...
                                    <p
                                        ...
                                        data-cy="questions-panel-p-added-by"
                                    >
                                ...
                                <div
                                    ...
                                    data-cy="questions-panel-div-38"
                                >
                                    <TooltipIconButton
                                        icon={Pencil}
                                        label="Edit"
                ...
                {filtered.length === 0 && (
                    <div
                        ...
                        data-cy="questions-panel-div-41"
                    >
                        <ShieldCheck
                            size={20}
                            className="mx-auto mb-2 text-neutral-300"
                            data-cy="questions-panel-shield-check-42"
                        />
                        {effectiveActiveSet
                            ? 'No questions match your search or filters.'
                            : 'Create a question set to get started.'}
                    </div>
                )}
            </div>
            ...
            <AddEditQuestionModal
                ...
                questionSet={effectiveActiveSet}
                data-cy="questions-panel-add-edit-question-modal-set-modal-open"
            />
            ...
            {pending && (
                <ConfirmDialog
                    ...
                    data-cy="questions-panel-confirm-dialog-set-pending"
                />
            )}
        </div>
    );
