...
export default function SeminarsPage() {
  ...
  function handleToggleAlert(key: SeminarAdminAlertSetting['key']) {
    setAdminAlerts((prev) => prev.map((a) => (a.key === key ? { ...a, enabled: !a.enabled } : a)))
  }
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-ink">Seminars</h1>
          <p className="text-sm text-neutral-500">
            {seminars.length} seminars · {participants.length} participants
          </p>
        </div>
        {tab === 'List of seminars' && (
      ...
      <ViewSeminarModal
        ...
        seminar={viewing}
        participants={participants}
        onViewParticipants={jumpToParticipants}
      />

      <p className="mt-4 text-[11px] text-neutral-400">Signed in as {currentUser.name}.</p>
    </div>
  )
}
