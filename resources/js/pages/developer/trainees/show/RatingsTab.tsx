...
export default function RatingsTab({ trainee }: { trainee: Trainee }) {
    const overall =
        trainee.taskRatings?.length > 0
            ? trainee.taskRatings?.reduce((sum, r) => sum + r.rating, 0) /
              trainee.taskRatings?.length
            : 0;
    const behavioral = behavioralRatingRecords.find(
        (r) => r.batchNo === trainee.batchNo && r.traineeId === trainee.id,
    );
    const behavioralScores = behavioral
        ? behavioral.answers.filter((a) => a.score != null)
        : [];
    const behavioralAvg = behavioralScores.length
        ? behavioralScores.reduce((sum, a) => sum + (a.score ?? 0), 0) /
        ...
        <>
            <TraineesDetailLayout trainee={trainee}>
                ...
                >
                    ...
                    <div
                        className="rounded-lg border border-neutral-200 bg-white p-5"
                        data-cy="ratings-tab-div-14"
                    >
                        <div
                            className="mb-3 flex items-center justify-between"
                            data-cy="ratings-tab-div-15"
                        >
                            <h3
                                className="text-sm font-semibold text-ink"
                                data-cy="ratings-tab-h3-behavioral-rating"
                            >
                                Behavioral rating
                            </h3>
                            {behavioral && (
                                <RatingStars
                                    value={behavioralAvg}
                                    size={13}
                                    data-cy="ratings-tab-rating-stars-17"
                                />
                            )}
                        </div>
                        {behavioral ? (
                            <div
                                className="rounded-md border border-neutral-200 p-3.5"
                                data-cy="ratings-tab-div-18"
                            >
                                {behavioral.overallComments && (
                                    <p
                                        className="text-xs leading-relaxed text-neutral-600"
                                        data-cy="ratings-tab-p-19"
                                    >
                                        {behavioral.overallComments}
                                    </p>
                                )}
                                {behavioral.recommendation && (
                                    <p
                                        className="mt-2 text-xs leading-relaxed text-neutral-500 italic"
                                        data-cy="ratings-tab-p-recommendation"
                                    >
                                        Recommendation:{' '}
                                        {behavioral.recommendation}
                                    </p>
                                ...
                                <div
                                    className="mt-2 text-xs text-neutral-400"
                                    data-cy="ratings-tab-div-evaluated-by-2"
                                >
                                    Evaluated by {behavioral.evaluator} on{' '}
                                    {behavioral.ratedAt}
                                </div>
                            </div>
                        ) : (
                            <div
                                className="rounded-md border border-dashed border-neutral-200 p-6 text-center text-sm text-neutral-500"
                                data-cy="ratings-tab-div-no-behavioral-rating-recorded-for-this"
                            >
