...
function PrintStars({ value }: { value: number }) {
  return (
    <span className="inline-flex items-center gap-0.5 align-middle">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star key={i} size={12} className={i <= Math.round(value) ? 'fill-ink text-ink' : 'text-neutral-300'} />
      ))}
...
export function BehavioralSheetPrint({ batchNo, rating, questions, average, generatedAt }: BehavioralSheetPrintProps) {
    ...
    <div className="hidden print:block print-area bg-white text-ink p-8">
      <header className="mb-5 flex items-center justify-between border-b-2 border-ink pb-4">
        ...
        <div className="text-right">
          <div className="text-base font-bold">Trainer Evaluation for Trainees</div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-x-6 gap-y-1.5 text-xs">
        ...
        <div><span className="font-semibold">Trainee:</span> {rating.traineeName}</div>
        <div><span className="font-semibold">Evaluator:</span> {rating.evaluator}</div>
        <div><span className="font-semibold">Date evaluated:</span> {rating.ratedAt}</div>
      </div>
      ...
      {sections.map(([section, qs]) => {
        ...
        return (
          <div key={section} className="mb-4">
            <div className="mb-1.5 text-[12px] font-bold">{section}</div>
            {isRatingSection ? (
              <table className="w-full border-collapse text-[11px]">
                <thead>
                  <tr className="border border-ink bg-neutral-100">
                    <th className="border border-ink px-2 py-1.5 text-left">#</th>
                    <th className="border border-ink px-2 py-1.5 text-left">Statement</th>
                    <th className="border border-ink px-2 py-1.5 text-left">Score</th>
                  </tr>
                </thead>
                <tbody>
                  {qs.map((q, i) => {
                    ...
                    return (
                      <tr key={q.id}>
                        <td className="border border-ink px-2 py-1.5">{i + 1}</td>
                        <td className="border border-ink px-2 py-1.5">{q.question}</td>
                        <td className="border border-ink px-2 py-1.5"><PrintStars value={answer?.score ?? 0} /></td>
                      </tr>
                    )
                  })}
                </tbody>
                ...
                {qs.map((q) => {
                  ...
                  return (
                    <div key={q.id} className="text-[11px]">
                      ...
                      <div className="mt-1 min-h-[32px] rounded border border-ink p-2">{answer?.text || '\u2014'}</div>
                    </div>
                  )
                })}
              </div>
        ...
        <table className="w-full border-collapse text-[11px]">
          <tbody>
            <tr className="bg-neutral-100 font-semibold">
              <td className="border border-ink px-2 py-1.5">Overall average (rated statements)</td>
              <td className="border border-ink px-2 py-1.5"><PrintStars value={average} /></td>
            </tr>
          </tbody>
        </table>
      )}
