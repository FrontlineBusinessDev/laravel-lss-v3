...
export function ScheduleFilters({
  ...
  options: { schools: string[]; programs: string[]; batches: string[]; industries: string[]; statuses: string[] }
}) {
  const hasActive =
    filters.schools.length + filters.programs.length + filters.batches.length + filters.industries.length + filters.statuses.length > 0
    ...
    <div className="mb-3 flex flex-col gap-2 rounded-lg border border-neutral-200 bg-white p-3 sm:flex-row sm:flex-wrap sm:items-center">
      <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap">
        <MultiSelectDropdown
          ...
          placeholder="School"
          onChange={(v) => onChange({ ...filters, schools: v })}
          className="sm:w-40"
        />
        <MultiSelectDropdown
          ...
          onChange={(v) => onChange({ ...filters, programs: v })}
          className="sm:w-44"
        />
        <MultiSelectDropdown
          ...
          placeholder="Batch"
          onChange={(v) => onChange({ ...filters, batches: v })}
          className="sm:w-36"
        />
        <MultiSelectDropdown
        ...
        />
        <MultiSelectDropdown
          options={options.statuses.map((s) => STATUS_LABELS[s] ?? s)}
          value={filters.statuses.map((s) => STATUS_LABELS[s] ?? s)}
          placeholder="Status"
          onChange={(labels) => {
            const reverse = Object.fromEntries(Object.entries(STATUS_LABELS).map(([k, v]) => [v, k]))
            onChange({ ...filters, statuses: labels.map((l) => reverse[l] ?? l) })
          }}
          className="sm:w-32"
        />
      </div>
