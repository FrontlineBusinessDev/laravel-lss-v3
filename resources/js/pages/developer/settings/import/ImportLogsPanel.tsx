...
import type { ImportLogRow } from '@/types/modules/developer/import-logs';

const GRID = 'sm:grid-cols-[1fr_1.6fr_1.3fr_1.3fr_1.1fr_2.5rem]';
const listHeader = (
    <SettingsListHeader
        grid={GRID}
        labels={['Type', 'File', 'Result', 'Imported by', 'Date']}
    />
);

/** Recent Settings > Import runs, each with an undo action scoped to exactly what that import created. */
export function ImportLogsPanel({ refreshKey = 0 }: { refreshKey?: number }) {
    const [rollingBackId, setRollingBackId] = useState<number | null>(null);
    const [rollbackErrors, setRollbackErrors] = useState<
        Record<number, string[]>
    >({});
    const [confirmTarget, setConfirmTarget] = useState<ImportLogRow | null>(
        null,
    );
    ...
    async function handleRollback(log: ImportLogRow) {
        ...
        try {
            ...
            setRollbackErrors((prev) => ({
                ...prev,
                [log.id]: ['Rollback failed — please try again.'],
            }));
        } finally {
    ...
    const renderRow = (log: ImportLogRow) => (
        <div key={log.id}>
            <SettingsRow
                ...
                menu={[
                        ...
                        : []),
                    {
                        ...
                        danger: true,
                        disabled:
                            !!log.rolled_back_at || rollingBackId === log.id,
            ...
            {detailsOpenId === log.id && log.errors?.length ? (
                <ul className="text-danger-700 mb-2 list-disc space-y-0.5 pl-8 text-xs">
                    {log.errors.map((e, i) => (
    ...
    return (
        <div className="mt-6">
            ...
            <ConfirmDialog
                ...
                onConfirm={() =>
                    confirmTarget && void handleRollback(confirmTarget)
                }
                title="Roll back this import?"
                description={
                    confirmTarget ? (
                        <>
                            This deletes the records created by the{' '}
                            <strong>{confirmTarget.type}</strong> import of{' '}
                            <strong>{confirmTarget.file_name}</strong>. Records
                            this import only matched (not created) are left
                            untouched.
                        </>
                    ) : null
                }
