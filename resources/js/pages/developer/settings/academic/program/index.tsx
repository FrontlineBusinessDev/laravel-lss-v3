...
const listHeader = <SettingsListHeader grid={customGRID} labels={['Name', 'Course Name', 'Specialization']} data-cy="index-settings-list-header-1" />;
const renderRow = (row: AcademicProgram, actions: CardActions) => {
  ...
  return <SettingsRow grid={customGRID} isArchived={isArchived} badge={<StatusBadge status={badge} data-cy="index-status-badge-3" />} menu={buildRecordMenu(actions, isArchived)} data-cy="index-settings-row-2">
            ...
            <TextCell data-cy="index-text-cell-6">{row.specialization}</TextCell>
        </SettingsRow>;
};
export default function index() {
  return <>
            <DataTableField<AcademicProgram> apiUrl="/settings/academic/program" apiQueryKey="settings-academic/program" columns={columns} fields={fields} createLabel="Add Program" modalTitle={s => s.mode === 'create' ? 'Add Program' : 'Edit Program'} defaultSortBy="first_name" createPermission="manage settings academic" editPermission="manage settings academic" archivePermission="manage settings academic" deletePermission="manage settings academic" listHeader={listHeader} renderCard={renderRow} data-cy="index-data-table-field-7" />
        </>;
}