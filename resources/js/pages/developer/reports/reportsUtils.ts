...
export { formatCurrency } from '@/pages/developer/payments/paymentsUtils';
 ...
 * payment data models.
 */
export function computeTotalEarnings(
    trainees: Trainee[],
    seminarParticipants: SeminarParticipant[],
): number {
    const traineeTotal = trainees.reduce(
        (sum, t) => sum + computePaymentBreakdown(t).totalAmountPaid,
        0,
    );
...
export function computeGroupFinancials(
    ...
    for (const t of traineeList) {
        ...
        if (t.status === 'completed' || t.completedHrs >= t.requiredHrs)
            completedCount += 1;
