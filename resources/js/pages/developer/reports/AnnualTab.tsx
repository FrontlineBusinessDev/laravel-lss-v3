...
import { seminarEarningsTotal } from '@/pages/developer/seminars/seminarUtils';
...
export function AnnualTab() {
    ...
    const [draftFilters, setDraftFilters] =
        useState<AnnualFilters>(EMPTY_FILTERS);
    ...
    const traineesByBatch = useMemo(() => {
        ...
        for (const b of batches) {
            map.set(
                b.batchNo,
                trainees
                    .filter((t) => t.batchNo === b.batchNo && !t.archived)
            ...
            .filter((b) => {
                if (appliedFilters.dateFrom || appliedFilters.dateTo) {
                    ...
                    if (
                        ...
                        started < new Date(appliedFilters.dateFrom)
                    )
                        return false;
                    if (
                        appliedFilters.dateTo &&
                        started > new Date(appliedFilters.dateTo)
                    )
                        return false;
                }
                if (!q) return true;
    ...
    const allFilteredTrainees = filteredBatches.flatMap(
        (b) => traineesByBatch.get(b.batchNo) ?? [],
    );
    ...
    const expandAll = () =>
        setExpanded(new Set(filteredBatches.map((b) => b.batchNo)));
    ...
    return (
        <>
            <div className="no-print mb-4 grid grid-cols-2 gap-3 sm:grid-cols-5">
                <StatCard
                    label="Batches"
                ...
                <StatCard
                    label="Total balance"
                    value={formatCurrency(overall.totalBalance)}
                    icon={Wallet}
                    tone={overall.totalBalance > 0 ? 'warning' : 'default'}
                />
                <StatCard
                    label="Seminar revenue"
                    value={formatCurrency(
                        seminarEarningsTotal(seminarParticipants),
                    )}
                    icon={Wallet}
                    tone="accent"
            ...
            <div className="no-print mb-3 flex flex-col gap-2.5 rounded-lg border border-neutral-200 bg-white p-3">
                ...
                <div className="flex items-start gap-1.5 text-[11px] text-neutral-400">
                    ...
                    Date range filters batches by their start date. Terminated
                    trainees remain in totals below.
                </div>
            </div>
            ...
            <div className="no-print mb-2 flex items-center justify-between">
                <span className="text-xs text-neutral-500">
                    ...
                    {filteredBatches.length === 1 ? '' : 'es'} &middot;{' '}
                    {dateRangeLabel}
                </span>
                        ...
                        >
                            ...
                            >
                                ...
                                {fin.completedCount > 0 && (
                                    <div className="text-success-700 flex items-center gap-1 text-xs">
                                        <CheckCircle2 size={13} />{' '}
                                        {fin.completedCount} completed
                                    </div>
                                )}
                                {fin.terminatedCount > 0 && (
                                    <div className="text-danger-700 flex items-center gap-1 text-xs">
                                        <UserX size={13} />{' '}
                            ...
                            {isOpen && (
                                <div className="lss-scrollbar overflow-x-auto border-t border-neutral-100">
            ...
            <AnnualReportPrint
                ...
                traineesByBatch={traineesByBatch}
