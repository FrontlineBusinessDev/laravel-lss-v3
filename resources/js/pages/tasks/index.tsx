...
const STATUS_LABEL: Record<string, string> = { open: 'Open', completed: 'Completed', locked: 'Locked' }
...
const EMPTY_FILTERS: ReportFilters = { dateFrom: '', dateTo: '', batch: '', trainees: [], trainers: [] }

/** Pending destructive/state-changing action awaiting confirmation. */
type PendingAction = { kind: 'complete' | 'lock' | 'delete'; task: TaskRecord } | null

const ACTION_COPY: Record<NonNullable<PendingAction>['kind'], { title: string; body: (t: string) => string; confirmLabel: string; tone: 'danger' | 'default' }> = {
  complete: { title: 'Mark task as complete', body: (t) => `Mark "${t}" as complete? The trainee's time spent will be locked in for reporting.`, confirmLabel: 'Mark complete', tone: 'default' },
  lock: { title: 'Lock task', body: (t) => `Lock "${t}"? Locked tasks can no longer be edited or completed.`, confirmLabel: 'Lock task', tone: 'default' },
  delete: { title: 'Delete task', body: (t) => `Delete "${t}"? This cannot be undone.`, confirmLabel: 'Delete', tone: 'danger' },
}
...
export default function TasksPage() {
  ...
  const managementRows = useMemo(
    () => records.filter((t) => (!managementBatchFilter || managementBatchFilter === 'Batch' ? true : t.batchNo === managementBatchFilter)),
  ...
  function confirmPendingAction() {
    ...
    if (kind === 'delete') {
      ...
      setRecords((prev) => prev.map((t) => (t.id === task.id ? { ...t, status } : t)))
  ...
  const updateTimeSpent = (id: string, value: number) => {
    setRecords((prev) => prev.map((t) => (t.id === id ? { ...t, timeSpent: value } : t)))
  }

  const updateRemarks = (id: string, value: string) => {
    setRecords((prev) => prev.map((t) => (t.id === id ? { ...t, remarks: value } : t)))
  }
    ...
    <div>
      <div className="mb-4 flex items-center justify-between no-print">
        <div>
          ...
          <p className="text-sm text-neutral-500">Daily trainee task assignment and reporting</p>
        </div>
        {tab === 'Task management' && (
      ...
      {tab === 'Task management' && (
        <>
          <div className="mb-3 flex flex-wrap items-center gap-2 rounded-lg border border-neutral-200 bg-white p-3">
            <Dropdown options={['Batch', ...batchNumbers]} placeholder="Batch" onChange={setManagementBatchFilter} className="sm:w-44" />
            <span className="ml-auto text-xs text-neutral-400">{managementRows.length} task{managementRows.length === 1 ? '' : 's'}</span>
          </div>
              ...
              <table className="w-full min-w-[880px] border-collapse text-sm">
                <thead>
                  <tr className="bg-neutral-50 text-left text-xs font-medium text-neutral-500">
                    ...
                    <th className="px-4 py-2.5 font-medium">Time goal</th>
                  ...
                  {managementRows.map((t) => (
                    <tr key={t.id} className="border-t border-neutral-100 transition-colors hover:bg-neutral-50">
                      <td className="px-4 py-2.5">
                        <span className={cn('inline-flex items-center rounded-pill px-2.5 py-0.5 text-xs font-medium', STATUS_STYLE[t.status])}>
                      ...
                      <td className="px-4 py-2.5 max-w-[220px] truncate text-xs text-neutral-500" title={t.description}>{t.description}</td>
                      <td className="px-4 py-2.5 font-mono text-xs text-neutral-600">{t.timeGoal}h</td>
                      <td className="px-4 py-2.5 text-neutral-600">{t.trainee}</td>
                      <td className="px-4 py-2.5 text-neutral-600">{t.trainer}</td>
                      <td className="px-4 py-2.5 font-mono text-xs text-neutral-600">{t.date}</td>
                      <td className="px-4 py-2.5">
                        <div className="flex justify-end gap-0.5">
                          <TooltipIconButton icon={FolderOpen} label="Open" onClick={() => setViewTask(t)} />
                          <TooltipIconButton
                            icon={CheckCircle2}
                            label="Complete"
                            disabled={t.status === 'completed' || t.status === 'locked'}
                            onClick={() => requestAction('complete', t)}
                          />
                          <TooltipIconButton
                            icon={Lock}
                            label="Lock"
                          ...
                          />
                          <TooltipIconButton icon={Trash2} label="Delete" danger onClick={() => requestAction('delete', t)} />
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="flex flex-col gap-2 sm:hidden">
            {managementRows.map((t) => (
              <div key={t.id} className="rounded-lg border border-neutral-200 bg-white p-3.5">
                <button onClick={() => setViewTask(t)} className="flex w-full items-start justify-between gap-2 text-left">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-ink">{t.task}</p>
                    <p className="truncate text-xs text-neutral-500">
                      {t.batchNo} · {t.trainee}
                    </p>
                  </div>
                  <span className={cn('shrink-0 rounded-pill px-2 py-0.5 text-[11px] font-medium', STATUS_STYLE[t.status])}>
                    {STATUS_LABEL[t.status]}
                  </span>
                </button>
                ...
                <div className="mt-2.5 flex gap-2 border-t border-neutral-100 pt-2.5">
                  <TooltipIconButton
                    ...
                    onClick={() => requestAction('complete', t)}
                  />
                  <TooltipIconButton icon={Lock} label="Lock" disabled={t.status === 'locked'} onClick={() => requestAction('lock', t)} />
                  <TooltipIconButton icon={Trash2} label="Delete" danger onClick={() => requestAction('delete', t)} />
                </div>
              </div>
            ))}
      ...
      {tab === 'Daily task sheet' && (
        <>
          <div className="mb-3 flex flex-col gap-3 rounded-lg border border-neutral-200 bg-white p-3 no-print">
            <div className="flex flex-wrap items-end gap-2">
              ...
              <div className="w-full sm:w-44">
                ...
                <Dropdown
                  ...
                  placeholder="Batch"
                  onChange={(v) => setDraftFilters((f) => ({ ...f, batch: v }))}
                />
              </div>
              <div className="w-full sm:w-52">
                <label className="mb-1 block text-[11px] font-medium text-neutral-500">Trainee</label>
                <MultiSelectDropdown
                  options={traineeNames}
                  value={draftFilters.trainees}
                  placeholder="Trainee"
                  onChange={(v) => setDraftFilters((f) => ({ ...f, trainees: v }))}
                />
              </div>
              <div className="w-full sm:w-52">
                <label className="mb-1 block text-[11px] font-medium text-neutral-500">Trainer</label>
                <MultiSelectDropdown
                  options={trainerNames}
                  value={draftFilters.trainers}
                  placeholder="Trainer"
                  onChange={(v) => setDraftFilters((f) => ({ ...f, trainers: v }))}
                />
              </div>
          ...
          <div className="mb-3 flex items-center justify-between no-print">
            <span className="text-xs text-neutral-500">{reportRows.length} record{reportRows.length === 1 ? '' : 's'}</span>
            <Button variant="secondary" size="sm" icon={Printer} onClick={() => window.print()} disabled={reportRows.length === 0}>
                ...
                <tbody>
                  {reportRows.map((t) => (
                    <tr key={t.id} className="border-t border-neutral-100">
                      <td className="px-4 py-2.5 font-mono text-xs text-neutral-600">{t.batchNo}</td>
                      <td className="px-4 py-2.5 font-medium text-ink">{t.task}</td>
                      <td className="px-4 py-2.5 max-w-[200px] truncate text-xs text-neutral-500" title={t.description}>{t.description}</td>
                      <td className="px-4 py-2.5 font-mono text-xs text-neutral-600">{t.timeGoal}h</td>
                      <td className="px-4 py-2.5">
                        {t.onLeave ? (
                          ...
                          <input
                            ...
                            min={0}
                            value={t.timeSpent}
                            onChange={(e) => updateTimeSpent(t.id, Number(e.target.value))}
                            className="h-8 w-16 rounded-md border border-neutral-200 px-2 font-mono text-xs focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-100"
                          />
                        )}
                      </td>
                      <td className="px-4 py-2.5 max-w-[220px]">
                        {t.onLeave ? (
                          <span className="inline-flex items-center rounded-pill bg-neutral-100 px-2 py-0.5 text-xs font-medium text-neutral-600" title={remarksFor(t)}>
                            {remarksFor(t)}
                          </span>
                        ) : (
                          <input
                            ...
                            onChange={(e) => updateRemarks(t.id, e.target.value)}
                            onBlur={(e) => {
                              if (e.target.value.trim() !== (t.remarks ?? '').trim()) commitRemarks({ ...t, remarks: e.target.value })
                            }}
                            className="h-8 w-full min-w-[140px] rounded-md border border-transparent px-2 text-xs text-neutral-600 transition-colors hover:border-neutral-200 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-100"
                          />
                        )}
                      </td>
                      <td className="px-4 py-2.5 text-neutral-600">{t.trainee}</td>
          ...
          <div className="mt-3 flex justify-end text-xs text-neutral-500 no-print">
            Total time spent (filtered): <span className="ml-1 font-mono font-semibold text-ink">{totalTimeSpent}h</span>
          </div>

          <DailyTaskSheetPrint rows={reportRows} generatedAt={printGeneratedAt} dateRangeLabel={dateRangeLabel} />
        </>
      )}
      ...
      <Modal open={!!viewTask} onClose={() => setViewTask(null)} title={viewTask?.task ?? ''} maxWidth={440}>
        {viewTask && (
          <div className="flex flex-col gap-3 text-sm">
            <div className="flex items-center gap-2">
              <span className={cn('inline-flex items-center rounded-pill px-2.5 py-0.5 text-xs font-medium', STATUS_STYLE[viewTask.status])}>
                {STATUS_LABEL[viewTask.status]}
              </span>
              <span className="font-mono text-xs text-neutral-500">{viewTask.batchNo}</span>
            </div>
            <p className="text-neutral-600">{viewTask.description}</p>
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 rounded-md bg-neutral-50 p-3 text-xs">
              <div><span className="text-neutral-500">Trainee</span><div className="font-medium text-ink">{viewTask.trainee}</div></div>
              <div><span className="text-neutral-500">Trainer</span><div className="font-medium text-ink">{viewTask.trainer}</div></div>
              <div><span className="text-neutral-500">Date</span><div className="font-mono font-medium text-ink">{viewTask.date}</div></div>
              <div><span className="text-neutral-500">Time goal</span><div className="font-mono font-medium text-ink">{viewTask.timeGoal}h</div></div>
              <div><span className="text-neutral-500">Time spent</span><div className="font-mono font-medium text-ink">{viewTask.onLeave ? '0h' : `${viewTask.timeSpent}h`}</div></div>
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-medium text-neutral-600">Remarks</label>
              {viewTask.onLeave ? (
                ...
                <textarea
                  ...
                  placeholder="Add remarks..."
                  rows={3}
                  onChange={(e) => updateRemarks(viewTask.id, e.target.value)}
                  className="w-full resize-none rounded-md border border-neutral-200 bg-white px-2.5 py-2 text-sm text-ink placeholder:text-neutral-400 transition-colors hover:border-neutral-300 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-100"
                />
              )}
            </div>
      ...
      <ConfirmDialog
        ...
        description={pendingAction ? ACTION_COPY[pendingAction.kind].body(pendingAction.task.task) : ''}
        confirmLabel={pendingAction ? ACTION_COPY[pendingAction.kind].confirmLabel : 'Confirm'}
        tone={pendingAction ? ACTION_COPY[pendingAction.kind].tone : 'default'}
      />
    </div>
  )
}
