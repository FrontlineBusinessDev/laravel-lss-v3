...
export function MiniCalendar({
  ...
  return <div className="rounded-lg border border-neutral-200 bg-white p-3.5" data-cy="mini-calendar-div-1">
      ...
      <div className="grid grid-cols-7 gap-y-1 text-center" data-cy="mini-calendar-div-10">
        {WEEKDAYS.map((w, i) => <div key={i} className="text-[10px] font-medium text-neutral-400" data-cy="mini-calendar-div-11">
            {w}
          </div>)}
        {cells.map((day, i) => {
        if (day === null) return <div key={i} data-cy="mini-calendar-div-12" />;
      ...
      <div className="mt-3 border-t border-neutral-100 pt-3" data-cy="mini-calendar-div-16">
        {selectedEvents.length > 0 ? <ul className="flex flex-col gap-2" data-cy="mini-calendar-ul-17">
            {selectedEvents.map(ev => <li key={ev.id} className="flex items-start gap-2 text-xs" data-cy="mini-calendar-li-18">
                ...
                <span className="text-neutral-700" data-cy="mini-calendar-span-20">{ev.title}</span>
              </li>)}
          </ul> : <p className="text-xs text-neutral-400" data-cy="mini-calendar-p-21">
