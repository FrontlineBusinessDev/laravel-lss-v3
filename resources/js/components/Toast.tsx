...
export function SystemToastProvider({ children }: { children: ReactNode }) {
    ...
    return (
        <ToastContext.Provider value={{ showToast }}>
            {children}
            {createPortal(
                <div className="pointer-events-none fixed inset-x-0 bottom-0 z-100 flex flex-col items-center gap-2 p-4 sm:right-4 sm:bottom-4 sm:left-auto sm:items-end">
                    {toasts.map((t) => {
                        ...
                        return (
                            <div
                                ...
                                role="status"
                                className={cn(
                                    'pointer-events-auto flex w-full max-w-sm animate-scaleIn items-start gap-2.5 rounded-lg border p-3 shadow-popover',
