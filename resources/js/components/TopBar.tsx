...
export function TopBar({ onOpenMenu }: { onOpenMenu: () => void }) {
    ...
    return (
        <header
            className="sticky top-0 z-30 flex h-12 items-center gap-3 border-b border-neutral-200 bg-white px-3 lg:hidden"
            data-cy="top-bar-header-1"
        >
            <button
                onClick={onOpenMenu}
                aria-label="Open menu"
                className="rounded-md p-1.5 text-neutral-600 transition-colors hover:bg-neutral-100 active:scale-95"
                data-cy="top-bar-button-open-menu"
            >
                <Menu size={18} data-cy="top-bar-menu-3" />
            </button>
            <div
                className="flex flex-1 items-center gap-1.5"
                data-cy="top-bar-div-4"
            >
            ...
            <div ref={ref} className="relative" data-cy="top-bar-div-8">
                <button
                    ...
                    data-cy="top-bar-button-set-open"
                >
                    {initials}
                </button>
                {open && (
                    <div
                        className="absolute top-[calc(100%+8px)] right-0 z-40 w-56 animate-scaleIn overflow-hidden rounded-lg border border-neutral-200 bg-white p-1 shadow-popover"
                        data-cy="top-bar-div-10"
                    >
                        <div
                            className="border-b border-neutral-100 px-3 py-2.5"
                            data-cy="top-bar-div-11"
                        >
                            <p
                                className="truncate text-sm font-semibold text-ink"
                                data-cy="top-bar-p-12"
                            >
                                {displayName}
                            </p>
                            <p
                                className="truncate text-xs text-neutral-500 capitalize"
                                data-cy="top-bar-p-13"
                            >
                                {role}
                            </p>
                        </div>
                        ...
                        <button
                            ...
                            className="flex w-full items-center gap-2.5 rounded-md px-3 py-2 text-left text-xs font-medium text-neutral-700 transition-colors hover:bg-neutral-50"
                            data-cy="top-bar-button-change-password"
                        >
                            <KeyRound
                                size={14}
                                className="shrink-0"
                                data-cy="top-bar-key-round-16"
                            />
                            Change password
                        </button>
                        <button
                            ...
                            data-cy="top-bar-button-set-open-3"
                        >
                            <LogOut
                                size={14}
                                className="shrink-0"
                                data-cy="top-bar-log-out-17"
                            />
                            Log out
                        </button>
                    </div>
