...
export function NotificationBell({
  ...
  return <>
      <button ref={btnRef} type="button" onClick={() => setOpen(v => !v)} aria-label={`Notifications${unreadAdminCount ? ` (${unreadAdminCount} unread)` : ''}`} aria-haspopup="menu" aria-expanded={open} className={cn('relative flex h-8 w-8 items-center justify-center rounded-md text-neutral-500 transition-colors hover:bg-neutral-100 hover:text-neutral-700', open && 'bg-neutral-100 text-neutral-700', className)} data-cy="notification-bell-button-button">
        <Bell size={17} data-cy="notification-bell-bell-2" />
        {unreadAdminCount > 0 && <span className="absolute -top-0.5 -right-0.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-danger-600 px-1 text-[9px] font-bold leading-none text-white" data-cy="notification-bell-span-3">
      ...
      width: 340
            ...
            <div className="flex items-center justify-between border-b border-neutral-100 px-3.5 py-2.5" data-cy="notification-bell-div-5">
              ...
              {unreadAdminCount > 0 && <button onClick={markAllRead} className="flex items-center gap-1 text-[11px] font-medium text-brand-600 transition-colors hover:text-brand-700" data-cy="notification-bell-button-mark-all-read">
            ...
            <div className="max-h-[54vh] overflow-y-auto lss-scrollbar" data-cy="notification-bell-div-9">
              {adminNotifications.length === 0 && <div className="px-4 py-8 text-center text-xs text-neutral-400" data-cy="notification-bell-div-no-notifications-yet">
                  <CalendarOff size={18} className="mx-auto mb-2 text-neutral-300" data-cy="notification-bell-calendar-off-11" />
                  No notifications yet.
                </div>}
              {adminNotifications.map(n => <button key={n.id} role="menuitem" onClick={() => openNotification(n.id, n.link)} className={cn('flex w-full items-start gap-2.5 border-b border-neutral-50 px-3.5 py-3 text-left transition-colors hover:bg-neutral-50', !n.read && 'bg-brand-50/40')} data-cy="notification-bell-button-open-notification">
