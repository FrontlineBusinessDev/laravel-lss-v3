...
export function Toolbar<T>({
    ...
    return (
        <div className="mb-4" data-cy="toolbar-div-1">
            {/* Top row: search + controls */}
            <div
                className="flex max-w-200 flex-col gap-3 sm:flex-row sm:items-center"
                data-cy="toolbar-div-2"
            >
                {/* Global search */}
                <div className="relative flex-1" data-cy="toolbar-div-3">
                    ...
                    {searchInput && (
                        <button
                            ...
                            data-cy="toolbar-button-clear-search"
                        >
                ...
                >
                    ...
                    {showFiltersButton && (
                        <button
                            ...
                            className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-2.5 text-sm font-medium transition-colors ${filtersOpen || hasActiveColumnFilters ? 'border-slate-300' : 'border-slate-200'}`}
                            data-cy="toolbar-button-button"
                        >
                            <SlidersHorizontal
                                className="h-3.5 w-3.5"
                                strokeWidth={1.75}
                                data-cy="toolbar-sliders-horizontal-10"
                            />
                            Filters
                        </button>
                    )}
                    ...
                    {sortableColumns.length > 0 && (
                        ...
                        >
                            <select
                                ...
                                data-cy="toolbar-select-sort-by-change"
                            >
                                {sortableColumns.map((c) => (
                                    <option
                                        key={c.key}
                                        value={c.key}
                                        data-cy="toolbar-option-sort"
                                    >
                                        Sort: {c.label}
                                    </option>
                                ))}
                            ...
                            <button
                                ...
                                data-cy="toolbar-button-button-2"
                            >
                                {sortDir === 'asc' ? (
                                    <ArrowUp
                                        className="h-3.5 w-3.5"
                                        data-cy="toolbar-arrow-up-15"
                                    />
                                ) : (
                                    <ArrowDown
                                        className="h-3.5 w-3.5"
                                        data-cy="toolbar-arrow-down-16"
                                    />
                                )}
                            </button>
                        </div>
                    ...
                    <select
                        ...
                        title="Rows per page"
                        data-cy="toolbar-select-rows-per-page"
                    >
                        {[10, 15, 25, 50, 100].map((n) => (
                            <option
                                key={n}
                                value={n}
                                data-cy="toolbar-option-page"
                            >
                                {n} / page
                            </option>
                        ))}
