...
export function StatusFilter({
  ...
  return <div className="inline-flex items-center rounded-xl border border-slate-200 p-0.5" data-cy="status-filter-div-1">
            {tabs.map(tab => <button key={tab.value} type="button" onClick={() => onChange(tab.value)} className={`rounded-lg px-3 py-2 text-sm font-medium transition-colors ${value === tab.value ? 'brand-400 text-white' : 'text-slate-600 hover:bg-slate-50'}`} data-cy="status-filter-button-button">
                    {tab.label}
                </button>)}
        </div>;
}