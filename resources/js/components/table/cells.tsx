...
export function TableAvatar({
    ...
    return (
        <span
            className={`inline-flex ${className} shrink-0 items-center justify-center rounded-full font-semibold text-white`}
...
const STATUS_STYLES: Record<string, { pill: string; dot: string }> = {
    ...
    invited: { pill: 'bg-amber-50 text-amber-700', dot: 'bg-amber-500' },
...
export function StatusPill({
    ...
    return (
        <span
            className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold capitalize ${style.pill}`}
        >
...
export function AdminTableHeader({
    ...
    return (
        <div
            className={`${gridClassName} border-b border-gray-100 bg-slate-50/60 py-3 text-[11px] font-semibold tracking-wider text-slate-400 uppercase`}
        >
            {children}
        </div>
    );
...
function IconButton({
    ...
    return (
        <button
            ...
            onClick={onClick}
            disabled={disabled}
            className={`inline-flex size-8 items-center justify-center rounded-lg border border-[#ecedf1] bg-white text-slate-500 transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${
                tone === 'danger'
    ...
    return (
        <div className="flex items-center justify-end gap-1.5">
            {isActive ? (
                <>
                    {actions.canEdit && (
                        <IconButton title="Edit" onClick={actions.onEdit}>
