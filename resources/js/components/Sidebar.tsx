...
export function Sidebar({ mobileOpen, onCloseMobile }: SidebarProps) {
  return (
    <>
      {mobileOpen && (
        <div
          className="fixed inset-0 z-40 bg-ink/40 animate-fadeIn lg:hidden"
      ...
      <aside
        className={cn(
          ...
          mobileOpen ? 'translate-x-0 animate-slideInLeft' : '-translate-x-full',
        )}
      >
        <div className="mb-3 flex items-center justify-between gap-2 px-2 py-1">
          ...
          <div className="flex items-center gap-1">
            <NotificationBell />
            <button onClick={onCloseMobile} className="rounded-sm p-1 text-neutral-400 hover:bg-neutral-100 lg:hidden" aria-label="Close menu">
              <X size={16} />
            </button>
          </div>
        </div>
        <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto lss-scrollbar">
...
function UserMenu() {
  ...
  return (
    <div ref={ref} className="relative mt-1 border-t border-neutral-100 pt-2">
      {open && (
        <div className="absolute bottom-[calc(100%+6px)] left-0 right-0 z-10 animate-scaleIn overflow-hidden rounded-lg border border-neutral-200 bg-white p-1 shadow-popover">
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block truncate text-sm font-semibold leading-tight text-ink">{displayName}</span>
          <span className="block truncate text-xs leading-tight text-neutral-500 capitalize">{role}</span>
        </span>
