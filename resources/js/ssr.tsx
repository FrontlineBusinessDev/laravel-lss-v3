...
createServerHtml(page => createInertiaApp({
  page,
  ...
  resolve: name => {
    ...
    const key = Object.keys(pages).find(k => k.toLowerCase() === `./pages/${name}.tsx`.toLowerCase());
  ...
  setup: ({
    ...
    return <AppProviders client={makeQueryClient()} data-cy="ssr-app-providers-1">
                    <App {...props} data-cy="ssr-app-2" />
                </AppProviders>;
  }
}));